...
module.exports = function (handler, dockerOpts, opts) {
    ...
    var monitorAll = true;
    if (opts && opts.strategy === 'monitorSelected') {
        monitorAll = false;
    }
    ...
    function monitoringEnabled(info) {
        if (monitorAll) {
            // monitor all containers that don't have label "node-docker-monitor"='disable'
            return !info.Labels || info.Labels['node-docker-monitor'] === undefined || info.Labels['node-docker-monitor'] !== 'disable';
        } else {
            // monitor only containers that have label "node-docker-monitor"='enable'
            return info.Labels && info.Labels['node-docker-monitor'] === 'enable';
        }
    }
